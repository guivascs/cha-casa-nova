...
app.get('/admin',admin,(req,res)=>res.render('admin',{gifts:db.prepare('SELECT g.id,g.name,g.reserved_by,g.reserved_at,u.name reserved_by_name FROM gifts g LEFT JOIN users u ON u.id=g.reserved_by ORDER BY g.id DESC').all()}));
app.post('/admin/gifts',admin,(req,res)=>{const name=(req.body.name||'').trim();if(name)db.prepare('INSERT INTO gifts(name) VALUES(?)').run(name);res.redirect('/admin');});app.post('/admin/gifts/:id/delete',admin,(req,res)=>{db.prepare('DELETE FROM gifts WHERE id=?').run(req.params.id);res.redirect('/admin');});app.post('/admin/gifts/:id/reset',admin,(req,res)=>{db.prepare('UPDATE gifts SET reserved_by=NULL,reserved_at=NULL WHERE id=?').run(req.params.id);res.redirect('/admin');});app.listen(PORT,()=>console.log(`Servidor rodando em http://localhost:${PORT}`));